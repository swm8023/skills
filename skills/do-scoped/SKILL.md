...
1. 按 `source_kind` 加载 `source`。approved-spec 先核对头部状态行，未标记 `已批准` 时（含状态行缺失的旧 spec）返回 scope，不在 do-scoped 内批准或改写；confirmed-conversation 核对当前对话中的完整确认记录；approved-fix 核对根因、证据、repro、fix scope、验收、验证、风险和诊断产物。
