...
用于用户主动点名的跨分支／worktree 检查与清单式操作。只要求检查或单独调用本 skill 时仅授权检查；改变 Git 状态前需要具体清单及覆盖它的用户授权。当前请求或既有批准已经明确覆盖同一清单时直接继承，不因这是本 skill 的初次调用而归零。一般性的“检查”或“继续看看”不授权提交、合入、推送、删除或清理。

日常实施收尾仍由 `git-workflow` 负责，不自动改走本 skill。对本 skill 管理的已授权清单，由本流程独占对应 Git 操作，不再叠加通用 prepare/checkpoint/finalize；只检查现有任务时不接管其 Git 生命周期。
...
核对清单与当前请求或既有批准；已有授权覆盖的操作不再确认，缺少授权的部分在展示具体清单后请求一次确认。默认不 push、不删除远端分支，除非清单明确列出且已有对应授权。用户只说“检查”“继续看看”或单独调用本 skill，不算执行确认。用户修改清单时复用其明确指令，仅对新增的未授权操作或实质歧义再次确认。

## 阶段二：执行已确认清单

### 1. 重验快照

重新检查所有相关 HEAD、status、upstream 和 worktree。与确认时快照不一致时先重新评估，不盲目执行旧清单；变化影响操作对象、提交内容、合入策略或风险时更新清单并确认差异，否则说明无实质影响并继续已有授权的操作。
