...
- `prepare` 读取配置、检查冲突并固化本次 Git 生命周期的有效决策；`checkpoint` 和 `finalize` 直接继承。用户明确修改本次决策时重新读取并确认；上下文缺失或不一致时停止并报告，新的 Git 生命周期由 `prepare` 重新读取。
...
1. 配置不存在时，按模板创建并写入当前日期，然后完成全部初始化问题；模板内容只是默认答案。
2. 配置存在时直接采用已明确项；本次生命周期所需项缺失、含糊或标记“待确认”时逐项询问并记录。
3. 用户对本次操作的明确指定优先，但不自动写回；只有用户要求长期采用时才更新配置。
...
1. 初始化先展示全部七个问题，再一次一题且不跳过；非初始化只问本次生命周期所需的未决项，也一次一题。
