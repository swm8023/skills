...
配置文件为本次受管仓库根目录下的 `docs/user/git-preferences.md`。

本 skill 负责普通仓库修改的 Git 生命周期。用户点名 `git-check` 后由其已授权清单管理的操作，以及 `pubwiki-markdown` 专用 helper 管理的数据仓库操作，不再叠加本 skill 的 prepare/checkpoint/finalize；不要让两个流程 stage、提交或收尾同一批变更。私有搜索索引位于仓库外，不启动 Git 生命周期。
...
- 配置不存在且需要初始化长期偏好：按模板建立配置并写入当前日期，复用当前对话已明确回答的项目，仅询问剩余初始化问题；模板本身不是授权。
- 所需项缺失、含糊或标记“待确认”：逐项询问并记录。
- 配置存在冲突：按下文规则一次处理一个。
- 用户明确要求修改长期偏好：具体值已明确时直接回写对应项，只询问尚未明确的部分。

用户仅指定本次操作时，直接覆盖当前上下文，不进入本流程、不回写；即使长期配置缺失，也不强迫先完成七项设置。仅询问当前操作确实需要且尚未明确的决策，其他未授权 Git 动作不执行。
...
1. 初始化先展示七项设置及已有答案，再一次一题询问剩余项；跳过当前对话已经回答或已接受的项目。非初始化只问本次生命周期所需的未决项或用户要求修改的长期偏好，也一次一题。
2. 每题附当前默认答案，允许用户回答“采用预设”；用户明确说“全部采用展示的预设”时一次接受已展示的整组设置，不再逐题重问。未展示或用户未接受的默认值不视为授权。
3. 回答后只更新对应操作和“最后更新”日期；回答不完整时记录已确认内容并将其余标记“待确认”，永久无需配置时记为“不适用”。
4. 每次写入后检查整份配置；有冲突时一次处理一个，列出冲突项和可选方案，用户选择后回写并复查。
5. 长期偏好初始化在全部项目已确认或不适用且无冲突后完成；只执行本次操作时按前述本次授权规则推进，不以长期初始化阻塞已明确的操作。
