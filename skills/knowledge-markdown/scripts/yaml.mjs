...
function stripComment(source) {
  ...
  for (let index = 0; index < source.length; index += 1) {
    ...
    if (character === '#' && (index === 0 || /\s/u.test(source[index - 1])) && stack.length === 0) {
...
function blockScalar(lines, start, parentIndent, folded, chomping) {
  ...
  for (let cursor = start; cursor < index; cursor += 1) {
    ...
    if (line.blank) values.push('');
    else values.push(line.raw.slice(Math.min(blockIndent, line.raw.length)));
  }
...
export function parseKnowledgeConfig(source, filename = 'knowledge.yaml') {
  ...
  if (config.version !== undefined && !((typeof config.version === 'number' && Number.isInteger(config.version)) || typeof config.version === 'string')) {
    throw new Error(`${filename} version must be a scalar`);
  }
  ...
  if (publish.mode !== undefined && typeof publish.mode !== 'string' && typeof publish.mode !== 'boolean') {
    throw new Error(`${filename} publish.mode must be a string or boolean`);
  }
  if (typeof publish.mode === 'string' && !['auto', 'manual', 'off', 'disabled', 'false'].includes(publish.mode.toLowerCase())) {
    throw new Error(`${filename} publish.mode has unsupported value: ${publish.mode}`);
  }
