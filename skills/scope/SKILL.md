...
用户确认理解一致后，请用户选择下一步路径：

> "下一步走哪条路径？
>
> - **落 spec** —— 我会写 `docs/scope/<日期>-<slug>/spec-<slug>.md`，等你审阅确认后按需同步 wiki，再调用 writing-plans 生成 plan
> - **不落 spec** —— 我直接在对话里生成 plan，你确认后立即执行（不留任何文档）"
...
3. 请用户审阅
4. 用户要求修改就改并重跑自我审查
5. 用户批准 spec 后、调用 writing-plans 前，同步 wiki：只扫描 `docs/wiki/` 路径和第一行摘要，判断 spec 中已确认的架构、约定、术语、稳定流程或决策背景是否需要沉淀。需要时自动更新匹配页面；没有匹配页面但值得沉淀时，按 spec 主题创建最小必要目录或页面，并更新索引。不要写入未确认猜测或一次性任务步骤
...
**终止状态：** 要么"对话内 plan → 执行"，要么"wiki 已按需同步 → 调用 writing-plans skill"。**不调用其他实施类 skill 作为 scope 的直接下一步。**
