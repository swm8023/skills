...
- **设计视图**：将决策组织成整体方案；系统结构、关键结构和关键流程按需展开，没有实质内容的直接省略。
- **预估改动面**：按模块或目录说明预计变化、测试范围和已确认的 wiki 目标；它是 do-scoped 的探索起点，不是硬边界。
- **验收**：逐项写明“场景或约束 → 可观察结果；验证证据”，覆盖正常行为、重要失败场景和兼容性。
- 每项结论只有一个权威定义；设计视图负责组织关系，验收负责验证结果，不重新定义需求或方案。
- 完整保留已确认的需求和技术结论；不写废弃备选、执行步骤或穷举式精确文件清单。

### 自我审查

1. **覆盖**：所有已确认需求和技术结论都有权威定义，且共同构成一个可整体验收的结果。
2. **分层**：需求边界、技术决策、设计视图和验收各司其职，没有矛盾或重复定义。
3. **交接**：设计足以让 do-scoped 理解入口、职责、状态、流转、失败收敛和兼容边界。
