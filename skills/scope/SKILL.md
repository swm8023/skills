...
根据当前对话和阶段 1 获得的事实建立内部**决策账本**，只跟踪影响需求边界和重大技术取舍的决策；每项标记为“已确认”“待决策”或“不适用”。局部、可逆且符合既有惯例的实现细节不进入待决策账本，由 implement 决定，不冒充用户已确认结论。账本只用于控制访谈和防止遗漏，不直接抄入实施契约或 spec。
...
- **代码库能答的不问用户**。
- **只澄清实质未决选择**。先用当前对话和代码事实消除不确定性；只有不同选择会实质影响需求边界、用户行为、兼容性、数据语义或重大技术取舍时，才标记为“待决策”交给用户。局部实现存在多种合理方式本身不构成提问理由，也不得用自主实现选择覆盖产品决策或硬约束。
