...
scope 处理尚未明确的需求和重大技术决策；局部实现细节留给 do-scoped。

如果目标、需求边界、技术决策、验收与验证已经齐全，只剩局部实现细节，不要制造额外问题：用一段最小摘要确认契约，问一个“是否按此契约执行”的确认；确认后进入阶段 3，通常推荐“不落 spec”。这样既保留实施闸门，也避免 scope 变成无止境的需求访谈。
...
进入阶段 2 时维护一份内部**决策账本**。每项标记为“已确认”“待决策”或“不适用”，并归入一个唯一类别：

- **目标**：当前问题、预期结果和用户价值。
- **需求边界**：支持对象与场景、可观察行为、失败行为、明确不做和必须保持的兼容行为。
- **技术决策**：职责与状态所有权、核心机制、接口、数据、存储、部署、迁移、安全和故障隔离中的高阶结论。
- **验收与验证**：完成条件、验证证据、测试边界和最终检查。
- **实施期信息**：精确文件、局部函数、任务顺序等留给 do-scoped 的内容，不进入 spec 决策基线。
- **wiki 目标**：更新、新建或不更新。

每次用户回答后先更新账本，再只问**最高影响的一个待决策项**；不要重问已确认内容，也不要为了填满清单而追问“不适用”项。所有适用项都闭合后，停止提问并进入阶段 3；如果用户推翻已确认决策，回到对应问题而不是继续沿用旧结论。

账本是防遗漏机制，不直接抄进 spec。生成实施契约时，每项已确认的需求或技术结论只能落入一个权威位置：需求边界写行为不变量，技术决策写方案约束，设计视图写它们之间的结构和关系，验收写完成证据。访谈过程、废弃备选和实施期信息不落 spec。
...
小需求若已经边界清晰，阶段 2 只做最小确认：复述目标、需求边界、验收和“不更新 wiki”的理由，提出一个是否执行的问题；不要为了形式再做代码库扫描或技术访谈。若仍缺少产品决策，按普通阶段 2 逐题收敛。
