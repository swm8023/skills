...
- **目标**：用一段话说明当前问题和预期结果，不重复边界或方案。
- **需求边界**：记录支持场景、正常与失败行为、明确不做及兼容要求；不讨论组件和文件。
- **技术决策**：记录职责、状态所有权、核心机制、接口、数据、存储、迁移、安全等 plan 不得自行改变的高阶结论；只为真实取舍保留必要理由。
- **功能设计**：将需求边界综合为完整功能，说明入口、交互、状态、正常与失败行为及功能间关系；不逐条复述需求。
- **技术设计**：将技术决策综合为端到端实现方案，说明系统如何实现功能设计；足以指导 do-scoped 细化实施，但不展开任务步骤。
- **整体方案**：说明系统组成、组件职责、状态与数据所有权、接口边界、依赖关系及协作方式。存在跨模块拓扑、数据流、控制流或状态迁移时，可使用 Mermaid 图辅助；图不代替关键职责和约束的文字说明。
- **关键结构 / 实现流程**：关键结构按需展开影响实现的数据模型、状态、接口或组件关系；实现流程从入口到结果写清主要调用、状态变化、异步处理、失败和清理，形成全局实施指导而不是任务清单。
- **预估改动面**：按模块或目录说明预计变化、测试范围和已确认的 wiki 目标；它是 do-scoped 的探索起点，不是硬边界。
- **验收**：逐项写明“场景或约束 → 可观察结果；验证证据”，覆盖正常行为、重要失败场景和兼容性。
- 每项结论只有一个权威定义；决策基线定义不可改变的需求边界和技术选择，设计视图负责将其综合为功能与技术方案，不引入未确认决策或与基线冲突。
- 完整保留已确认的需求和技术结论；不写废弃备选、执行步骤或穷举式精确文件清单。
