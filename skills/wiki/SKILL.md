...
例外：`scope` skill 在出口选择时已向用户列出并获得确认的 wiki 文件，视为已授权，不再重复确认。已批准 spec 的同步可以更新或创建用户确认过的最小必要 wiki；已确认对话内 plan 的小任务只能更新用户确认过的已有 wiki，不能自动创建新目录或页面。两者仍必须遵守摘要扫描、渐进读取、只写稳定知识和更新索引的规则。
